...
public interface ReportRepository extends JpaRepository<Order, Integer> {
    ...
    // Ưu tiên: Họ tên thật -> Tên đăng nhập -> N'Chưa phân công'
    @Query(value = "SELECT TOP 5 COALESCE(NULLIF(u.full_name, ''), u.username, N'Chưa phân công') as shipperName, COUNT(o.order_id) as totalOrders " +
            "FROM Orders o LEFT JOIN Users u ON o.shipper_id = u.user_id " +
            "WHERE o.status_id = 3 AND o.order_date >= :startDate AND o.order_date <= :endDate " +
            "GROUP BY COALESCE(NULLIF(u.full_name, ''), u.username, N'Chưa phân công') ORDER BY totalOrders DESC", nativeQuery = true)
    ...
    // Ưu tiên: Họ tên thật -> Tên đăng nhập -> N'Khách lẻ'
    @Query(value = "SELECT TOP 5 COALESCE(NULLIF(u.full_name, ''), u.username, N'Khách lẻ') as customerName, SUM(o.final_amount) as totalSpent " +
            "FROM Orders o LEFT JOIN Users u ON o.customer_id = u.user_id " +
            "WHERE o.status_id = 3 AND o.order_date >= :startDate AND o.order_date <= :endDate " +
            "GROUP BY COALESCE(NULLIF(u.full_name, ''), u.username, N'Khách lẻ') ORDER BY totalSpent DESC", nativeQuery = true)
    ...
    // 5. Lấy danh sách TẤT CẢ sản phẩm đã bán, SẮP XẾP THEO DOANH THU giảm dần
    ...
    // 6. Lấy danh sách TẤT CẢ sản phẩm đã bán, SẮP XẾP THEO SỐ LƯỢNG giảm dần
    @Query(value = "SELECT p.name as productName, SUM(od.quantity) as totalSold, SUM(od.quantity * od.price) as totalRevenue " +
            "FROM OrderDetails od JOIN Orders o ON od.order_id = o.order_id " +
            "JOIN Products p ON od.product_id = p.product_id " +
            "WHERE o.status_id = 3 AND o.order_date >= :startDate AND o.order_date <= :endDate " +
    ...
    @Query(value = "SELECT COALESCE(NULLIF(u.full_name, ''), u.username, N'Khách lẻ') as customerName, SUM(o.final_amount) as totalSpent " +
            "FROM Orders o LEFT JOIN Users u ON o.customer_id = u.user_id " +
            "WHERE o.status_id = 3 AND o.order_date >= :startDate AND o.order_date <= :endDate " +
            "GROUP BY COALESCE(NULLIF(u.full_name, ''), u.username, N'Khách lẻ') ORDER BY totalSpent DESC", nativeQuery = true)
    ...
    @Query(value = "SELECT COALESCE(NULLIF(u.full_name, ''), u.username, N'Chưa phân công') as shipperName, COUNT(o.order_id) as totalOrders " +
            "FROM Orders o LEFT JOIN Users u ON o.shipper_id = u.user_id " +
            "WHERE o.status_id = 3 AND o.order_date >= :startDate AND o.order_date <= :endDate " +
            "GROUP BY COALESCE(NULLIF(u.full_name, ''), u.username, N'Chưa phân công') ORDER BY totalOrders DESC", nativeQuery = true)
    ...
    // Đã thay đổi: Lấy o.order_code thay vì o.order_id để hiển thị mã TZG đồng bộ với Quản lý đơn hàng.
    // Đã thay đổi: Áp dụng COALESCE để bắt tên khách chuẩn xác.
    @Query(value = "SELECT o.order_code as orderId, COALESCE(NULLIF(u.full_name, ''), u.username, N'Khách lẻ') as customerName, o.order_date as orderDate, o.final_amount as finalAmount " +
            "FROM Orders o LEFT JOIN Users u ON o.customer_id = u.user_id " +
            "WHERE o.status_id = 3 AND o.order_date >= :startDate AND o.order_date <= :endDate " +
    ...
    // 10. Thống kê số lượng đơn hàng theo từng tháng
    ...
    // 11. TRÙM CUỐI: Lấy số liệu Tổng Quan
    @Query(value = "SELECT " +
